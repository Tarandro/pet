...
class PVP(ABC):
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False) \
        ...
        parts_a = [(tokenizer.encode(x, add_special_tokens=False, max_length=512, **kwargs), s) for x, s in parts_a if x]

        if parts_b:
            parts_b = [x if isinstance(x, tuple) else (x, False) for x in parts_b]
            parts_b = [(tokenizer.encode(x, add_special_tokens=False, max_length=512, **kwargs), s) for x, s in parts_b if x]
...
class BinaryPolarity2PVP(PVP):
    """
    Example for a pattern-verbalizer pair (PVP).
    """

    # Set this to the name of the task
    TASK_NAME = "binary-polarity2"
...
PVPS = {
    ...
    "binary-polarity2": BinaryPolarity2PVP
}
